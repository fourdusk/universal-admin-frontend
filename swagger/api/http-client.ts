...
export const responseErrorInterceptor = (err: AxiosError) => {
  const codeActions = [
    {
      code: 401,
  ...
  const codeAction = codeActions.find(o => o.code === code)
  const messageHtml = `<div>错误代码：${code}<div><div style="margin-top: 0.5rem;">错误消息：${message}</div>`
  if (codeAction) {
    codeAction.action(messageHtml)
  } else {
    ElMessage.error({
      dangerouslyUseHTMLString: true,
      message: messageHtml
